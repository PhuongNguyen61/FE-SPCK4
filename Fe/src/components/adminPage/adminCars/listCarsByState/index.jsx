...
const ListCarsByState = () => {
    ...
    const queryCountCars = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`http://localhost:8080/api/v1/cars/countCarsByState/${splitPathname[5]}`,
                {
                    headers: {
    ...
    const queryListCars = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`http://localhost:8080/api/v1/cars/carByState?limit=${limit}&currentPage=${currentPage}&isStatus=${isStatus}&state=${splitPathname[5]}`,
                {
                    headers: {
                        'Authorization': `Bearer ${accessToken}`,
                    },
    ...
    return (
        <div className='listCarsByState'>
            <div className='head'>
                <h3>Xe theo tình trạng: {getStateName(splitPathname[5])}</h3>
                {/* <h5>Xem</h5> */}
            </div>
            <div className='carsFilter'>
                <div className={pathname === `/admin/cars/carsByState/all/${splitPathname[5]}` ? activeDiv : div} onClick={() => navigate(`/admin/cars/carsByState/all/${splitPathname[5]}`)}>
                    <p>Tất cả</p>
                    <p>({totalCars ? totalCars : 0})</p>
                </div>
                |
                <div className={pathname === `/admin/cars/carsByState/approved/${splitPathname[5]}` ? activeDiv : div} onClick={() => navigate(`/admin/cars/carsByState/approved/${splitPathname[5]}`)}>
                    <p>Đã duyệt</p>
                    <p>({approvedCars ? approvedCars : 0})</p>
                </div>
                |
                <div className={pathname === `/admin/cars/carsByState/pending/${splitPathname[5]}` ? activeDiv : div} onClick={() => navigate(`/admin/cars/carsByState/pending/${splitPathname[5]}`)}>
                    <p>Chưa duyệt</p>
...
export default ListCarsByState