...
const ListRegisterProvider = () => {
    ...
    const queryCountApplys = async () => {
        setLoading(true);
        try {
            const response = await axios.get('http://localhost:8080/api/v1/applications/countApplys',
                {
                    headers: {
    ...
    const queryListApplys = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`http://localhost:8080/api/v1/applications/getAllApplys?limit=${limit}&currentPage=${currentPage}`,
                {
                    headers: {
                        'Authorization': `Bearer ${accessToken}`,
                    },
    ...
    const handleApproveApply = async (id) => {
        setLoading(true);
        try {
            const response = await axios.put(`http://localhost:8080/api/v1/applications/approve/${id}`,
                {},{
                    headers: {
                        'Authorization': `Bearer ${accessToken}`,
                    },
                            ...
                            return <div key={idx + 1} className='tr' style={apply.isStatus === "pending" ? {backgroundColor:'#2271B120'} : {backgroundColor:'#FFFFFF'}}>
                                <div className='info'>
                                    ...
                                    <div className='otherInfo'>
                                        ...
                                        <div className='row'>
                                            <h5>Ngày sinh:</h5>
                                            <i>{moment(apply.userId.dateOfBirth).format('DD/MM/YYYY')}</i>
                                        </div>
                                        <div className='row'>
                                            <h5>Email:</h5>
                                ...
                                <p className='time'>{moment(apply.updatedAt).format('HH:mm, DD/MM/YYYY')}</p>
